...
public class GradeController {
    ...
    @PostMapping
    public ResponseEntity<Void> createGrade(@RequestBody GradeInDTO gradeInDTO) {
        gradeService.createGrade(gradeInDTO);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }

    @PutMapping("/{id}")
    public ResponseEntity<Void> updateGrade(@RequestBody GradeInDTO gradeInDTO, @PathVariable Long id) {
